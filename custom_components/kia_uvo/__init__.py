...
async def async_setup(hass: HomeAssistant, config_entry: ConfigEntry):
    ...
    async def async_handle_force_update(call):
        ...
        self.hass.async_create_task(vehicle.force_update())

    async def async_handle_update(call):
        vehicle: Vehicle = hass.data[DOMAIN][DATA_VEHICLE_INSTANCE]
        self.hass.async_create_task(vehicle.update())
    ...
    async def async_handle_start_climate(call):
        ...
        vehicle: Vehicle = hass.data[DOMAIN][DATA_VEHICLE_INSTANCE]
        self.hass.async_create_task(vehicle.start_climate(set_temp, duration, defrost, climate, heating))

    async def async_handle_stop_climate(call):
        vehicle: Vehicle = hass.data[DOMAIN][DATA_VEHICLE_INSTANCE]
        self.hass.async_create_task(vehicle.stop_climate())

    async def async_handle_start_charge(call):
        vehicle: Vehicle = hass.data[DOMAIN][DATA_VEHICLE_INSTANCE]
        self.hass.async_create_task(vehicle.start_charge())

    async def async_handle_stop_charge(call):
        vehicle: Vehicle = hass.data[DOMAIN][DATA_VEHICLE_INSTANCE]
        self.hass.async_create_task(vehicle.stop_charge())
